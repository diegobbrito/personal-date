  ...
  const handleSelectPackage = (packageType: "simple" | "complete") => {
    ...
    setFormData(
      packageType === "complete"
        ? [
            { ...formData[0] },
            { sender: "", eventDate: "", eventTime: "", message: "", address: "", fontFamily: "Arial, sans-serif", template: "classic" },
          ]
        : [formData[0]]
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-900 to-purple-900">
      <div className="flex flex-col items-center py-5 lg:py-12 px-4 sm:px-6 mx-auto w-full">
        <h1 className="text-4xl md:text-6xl text-white font-mono text-center">Crie seu Convite!</h1>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {formData.map((invite, index) => (
              <div key={index} className="border border-white/20 p-6 mb-6 rounded-lg bg-white/5">
                ...
                <InviteInputField label="Remetente" name="sender" value={invite.sender} onChange={(e) => handleChange(e, index)} placeholder="Quem está enviando este convite?" id={""} />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InviteInputField label="Data do Evento" name="eventDate" type="date" value={invite.eventDate} onChange={(e) => handleChange(e, index)} id={""} />
                  <InviteInputField label="Horário" name="eventTime" type="time" value={invite.eventTime} onChange={(e) => handleChange(e, index)} id={""} />
                </div>
                <InviteInputField label="Endereço" name="address" value={invite.address} onChange={(e) => handleChange(e, index)} placeholder="Onde será o evento?" id={""} />
                <InviteSelectField label="Estilo da Fonte" name="fontFamily" value={invite.fontFamily} onChange={(e) => handleChange(e, index)} options={fontOptions} id={""} />
                <InviteSelectField label="Modelo do Convite" name="template" value={invite.template} onChange={(e) => handleChange(e, index)} options={templateOptions} id={""} />
                <InviteTextAreaField label="Mensagem Personalizada" name="message" value={invite.message} onChange={(e) => handleChange(e, index)} placeholder="Escreva sua mensagem especial..." rows={8} id={""} />
              </div>
            ))}
            <button type="submit" className="w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 shadow-lg">Gerar Convite</button>
          </form>
        </div>
